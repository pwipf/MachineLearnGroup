...
public void populationCrossover(double crossoverRate)
{
...
population.set(i, crossover(population.get(i), population.get(secondParent), crossoverRate));
}
}
	...
	for(int l=1;l<sizes.length;l++){
		for(int i=0;i<sizes[l];i++){
			for(int j=0;j<sizes[l-1];j++){
				if (Math.random() <= .5)
                                {
                                temp.w[l][i][j]=population.get(z).w[l][i][j] + beta*(population.get((int)Math.random()*population.size()).w[l][i][j]);
                                }
                                else
                                {
                                temp.w[l][i][j]=population.get(z).w[l][i][j] - beta*(population.get((int)Math.random()*population.size()).w[l][i][j]);    
                                ...
                                }
{
                       ...
                       if (Math.random() <= .5)
                                {
			temp.b[l][i]=population.get(z).b[l][i] + beta*(population.get((int)Math.random()*population.size()).b[l][i]);
                                }
                       else
                       {
                       temp.b[l][i]=population.get(z).b[l][i] - beta*(population.get((int)Math.random()*population.size()).b[l][i]);    
